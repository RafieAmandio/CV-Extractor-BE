...
class SimpleCVScoring {
  ...
  async initialize() {
    try {
      this.log('info', 'Initializing Simple CV Scoring...');
      ...
      this.csvPath = path.join(this.outputDir, `cv-scores-simple-${timestamp}.csv`);
  ...
  /**
   * Convert PDF to images and score against all jobs using multimodal LLM
   */
  async scoreCV(cvFile) {
    ...
    try {
      ...
      const imageBuffers = await pdfService.extractImages(cvFile.filepath);
      
      if (imageBuffers.length === 0) {
        throw new Error('No images could be extracted from PDF');
      }

      // Step 2: Convert images to data URLs
      const imageDataUrls = imageBuffers.map(buffer => 
        `data:image/png;base64,${buffer.toString('base64')}`
      );

      // Step 3: Create content with job descriptions and images
      const content = [
        {
          type: "text",
          text: `You are an expert CV analyzer and job matching specialist. 
...
Read the CV images carefully and provide comprehensive analysis:`
      ...
      // Step 4: Call OpenAI with multimodal input
      ...
      // Step 5: Write results to CSV immediately
  ...
  async run() {
    try {
      ...
      console.log('╔═══════════════════════════════════════════════╗');
      console.log('║        🎯 SIMPLE CV SCORING 🎯                ║');
      console.log('║    PDF → Images → LLM → CSV (Real-time)      ║');
      console.log('╚═══════════════════════════════════════════════╝');
      ...
      console.log(`${colors.yellow}   • Method: Multimodal LLM (PDF → Images)${colors.reset}`);
      ...
      console.log('🎉 SIMPLE CV SCORING COMPLETE!');
      ...
      this.log('error', 'Simple CV scoring failed', { error: error.message });
...
if (require.main === module) {
  const scorer = new SimpleCVScoring();
  scorer.run().catch(console.error);
}

module.exports = SimpleCVScoring; 